...
Future<void> doParallelDownloadTask_custom(
  ...
  if (!isResume) {
    ...
    if (!task.hasFilename) {
      task =
      (await taskWithSuggestedFilename(task, headers, true))
    ...
    chunks = createChunks_custom(task);
...
List<Chunk> createChunks_custom(ParallelDownloadTask task) {
  try {
    final numChunks = task.urls.length * task.chunks;
    final contentLength = task.totalBytes;
    if (contentLength <= 0) {
      throw StateError(
